---
name: to-issues
description: Step 4 of the spec pipeline. Decomposes SPEC.md into an issues/ folder — one file per issue — and on re-runs appends new issues without touching existing ones. Use when the user asks to break a spec into issues, tasks, or a plan, or to update the issues after a spec change. Fresh conversation; plans only, writes no code.
---
...
as ONE FILE PER ISSUE in `specs/S<nn>-<slug>/issues/`. There is no index
file: the zero-padded filenames sorted alphabetically ARE the build order.
You plan from the spec only — you do not design new behavior, and you do
not write code.
...
- Every issue must cite the spec sections and the section 11 acceptance
  ...
  is scope creep — delete it, or record the missing criterion as an open
  question (see below).
- Questions you DISCOVER during decomposition (a missing criterion, a
  contradiction, a gap that maps to no single issue) get APPENDED to
  SPEC.md section 13 (Open Questions) — that is where unblock looks.
  Section 13 is the only thing outside issues/ you may touch, and only by
  appending. A gap that blocks a specific issue still gets
  `status: BLOCKED:` on that issue.
...
- SAVE every issue file with your file-writing tool — printing is not
  saving. Then VERIFY: list the issues/ folder and confirm every file
...
per D31). Nothing renumbered, no status reset.
...
4. Every spec section 13 open question that affects an issue shows up as
   BLOCKED on that issue; every question you discovered during decomposition
   has been appended to SPEC.md section 13.
5. Every issue file you claim to have written appears in the issues/ folder
   listing (this catches unsaved files).
...
One file per issue; the padded filenames, sorted, are the build order.
Every issue is a vertical slice — thin end-to-end behavior, never a layer;
the first is the walking skeleton. Every criterion lands in an issue; every
issue bounds its Files; every gap becomes BLOCKED, never a guess; every
discovered question is appended to SPEC.md section 13. Save with real tool
calls and verify the folder listing. On re-runs, existing files are
untouchable.
